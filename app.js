...
async function dbDelete(table, matchColumn, matchValue) { try { const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${matchColumn}=eq.${matchValue}`, { method: "DELETE", headers: defaultHeaders }); if (!res.ok) throw new Error(await res.text()); return true; } catch (e) { console.error(`Error deleting from ${table}:`, e); return false; } }
...
const AppService = {
    getRuns: async () => {
        ...
        for(const run of rawRuns) {
            // Skip runs already marked as exported
    ...
    handleExpiredRun: async (runId, displayLabel) => {
        // Check if already exported to avoid spamming
        ...
        if (participants.length === 0) {
            // If no one registered, just delete or mark as empty? 
            // Mark as exported anyway to stop the interval
        ...
        try {
            ...
            if (res.ok) {
                // INSTEAD OF DELETE: Mark as exported so it stays in history!
                await dbUpdate('stride_runs', 'id', runId, { date_label: `[EXPORTED] ${displayLabel}` });
            }
        } catch(e) {
    ...
    getPastUserRuns: async (userId) => {
        ...
        // Map registrations to run details
        return regs.map(reg => {
