    ...
    submitOrder: async (itemId, size, phone, method, detail, photoFile) => {
        ...
        if (res) {
            (async () => {
                ...
                try {
                    const fd = new FormData(); fd.append('chat_id', chatId); fd.append('caption', cap); fd.append('parse_mode', 'Markdown');
                    if (photoFile) { fd.append('photo', photoFile); await fetch(`https://api.telegram.org/bot${botToken}/sendPhoto`, { method: 'POST', body: fd }); }
                    else { await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: cap, parse_mode: 'Markdown' }) }); }
                } catch (e) {}
            })();
            return true;
        }
